...
try:
    ...
    main_window_1 = app.window(title_re="^Inventory -.*$")
    ...
    if app:
        ...
        for attempt in range(30):  # check up to ~30 seconds
            try:
                ...
                main_window = app_1.window(title_re="^Update Stores Inventory -.*$")
        ...
        #find_element(automation_id="SplitContainer1", control_type="Pane", action="find" ,  retries=30, single_attempt_timeout=60, retry_interval=0,main_window=main_window)
        #wait_until_element_present(automation_id="SplitContainer1", control_type="Window", retries=30, single_attempt_timeout=60, retry_interval=0)
        ...
        for auto_id in ("dtFromDate", "dtToDate"):
            ...
            for attempt in range(1, 4):  # try 3 times
                try:
                    t0 = time.time()
                    pane = find_element(automation_id=auto_id, control_type="Pane",action="find", main_window=main_window)
                    pane.set_focus()
                    send_keys('{SPACE}')
        ...
        # Click Search 
        find_element(automation_id="cmdSearch", control_type="Button", action="click", main_window=main_window)

        wait(3)

        filtered_sorted_data = [
            {"Material Code": " 519062 ", "Material Description": "Test_25", "Deployment Date": "2025-08-04", "RPA Definition Remarks - GT": "pending"},
            #{"Material Code": " 328660 ", "Material Description": "Huawei Nova 8i Black_SA", "Deployment Date": "2025-08-04", "RPA Definition Remarks - GT": "pending"},
             {"Material Code": " 0324443 ", "Material Description": "APP_Marvel AV ZIP WindbreakerJacket WhS", "Deployment Date": "2025-08-04", "RPA Definition Remarks - GT": "pending"},
        ]

        for row in filtered_sorted_data:
            matcode = row.get("Material Code").strip()
